...
exports.generateOtp = async (request, response) => {
    ...
    //     response.send({ status: SendOtpToUser.status, token: lastToken , mobile : mMobile })
    // }
    // else {
    //     response.send({ status: JSON.stringify(request.body) })
    // }
}
        ...
        if (err) {
            ...
            if (userCode === getOtp.code) {
                ...
                if (inUser != null && inUser._id === mMobile) {
                    res.send({ token: token, status: 200, oldUser: true, mobile: mMobile });
                } else {
